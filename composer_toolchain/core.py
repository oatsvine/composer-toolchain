...
class ScoreSpec(BaseModel):
    ...
    # INVARIANT: normalize score must guarantee all parts have same measure structure.

    # DONE: Covered via test_score_cli_spec_multimovement using the Jupiter fragment (score.1-314.xml).
    movements: Dict[int, str] = Field(
        description="Map movement number to movement title"
    )
    # DONE: music21 exposes active TimeSignature via measure context, so measure-indexed map is stable.
    ...
    # DONE: Referenced MetronomeMark docs; .number already yields BPM so we store rounded integers per beat.
...
class Context:
    ...
    @staticmethod
    def init_from_score(score_file: Path, cwd: Path) -> Path:
        """
        ...
        workspace_scores = work_dir / "scores"
        workspace_scores.mkdir(exist_ok=True)
    ...
    def _master_file(self) -> Path:
        ...
        master_file = self.work_dir / "scores" / manifest.master
    ...
    def _bump(self, relative: Path) -> int:
        ...
        versions_root = self.work_dir / "versions" / relative.parent
    ...
    def _supersede(self, src_file: Path) -> Optional[Path]:
        ...
        archive_dir = self.work_dir / "versions" / relative.parent
    ...
    def _save(self, score: Score, suffix: str = ".krn") -> Optional[Path]:
        """Saves the score to well-known master file path; backs up previous version."""
        master_file = self._master_file()
    ...
    def merge_excerpt(
        ...
        master_tokens = {self._canonical_part_id(p) for p in score.parts}
        master_tokens.update(str(p.id) for p in score.parts if p.id is not None)
        excerpt_ids = [self._canonical_part_id(p) for p in excerpt_score.parts]
        ...
        measure_spec = MeasureSpec(spec=self._numbers_to_spec(measure_numbers))
    ...
    def _build_spec(self, score: Score) -> ScoreSpec:
        meta = score.metadata
        ...
        for part in score.parts:
            part_id = (
                str(part.id) if part.id is not None else self._canonical_part_id(part)
            )
        ...
        for gc in score.getElementsByClass(GlobalComment):
            ...
            # DONE: music21 hierarchy docs recommend getOffsetBySite; fallback keeps derived comments addressable.
            try:
